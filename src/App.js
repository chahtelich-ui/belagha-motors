...
export default function App() {
  ...
  const startCamera = async (mode) => {
    ...
    try {
      ...
      // طلب الكاميرا الخلفية للرخصة والأمامية للمستأجر
      const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: mode === 'tenant' ? "user" : "environment" } });
      streamRef.current = stream;
      if (videoRef.current) {
        videoRef.current.srcObject = stream;
        // خاصية حيوية لعمل الكاميرا في iOS Safari
        videoRef.current.setAttribute("playsinline", "true");
        videoRef.current.play();
      }
    } catch (err) {
      console.error(err);
      alert("⚠️ المتصفح يمنع الكاميرا. الرجاء الذهاب لإعدادات الأيباد > Safari > الكاميرا > والسماح بها.");
      setCameraMode(null);
    }
  };
  ...
  const handleFileUpload = (e, mode) => {
    ...
    // تصفير الخانة للسماح برفع نفس الصورة مجدداً بدون مشاكل
    e.target.value = '';
  };
  ...
  const executePureVisionAI = async (base64Image) => {
    ...
    try {
      ...
      const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: [
              { type: "text", text: "You are an expert OCR. Extract data from this Algerian driver license. Return ONLY a pure JSON object (no markdown). Keys must be: tenantName (Latin uppercase), licenseNumber (18 digits), birthDate (DD.MM.YYYY), issueDate (DD.MM.YYYY)." },
      ...
      console.error(err);
      alert(err.message.includes("رصيد") ? err.message : "⚠️ تعذر استخراج البيانات بدقة. يرجى التأكد من وضوح الصورة وصلاحية المفتاح.");
  ...
  const handleOriginalPrintSubmit = (e) => {
    ...
    // تأخير آمن للـ iPad لضمان بناء الصفحات الثلاثة قبل فتح أمر الطباعة
    setTimeout(() => { 
      window.print(); 
            ...
            <div style={styles.card}>
              <form onSubmit={handleOriginalPrintSubmit}>
                <div style={styles.photoGrid}>
                  {/* قسم المستأجر - تم حل مشكلة السفاري هنا */}
                  <div style={styles.photoSection}>
                    ...
                    <div style={styles.photoBox}>
                      ...
                      <div style={styles.photoActions}>
                        ...
                        <label htmlFor="tenant-upload-btn" style={styles.btnSecondary}>📂 ملف جاهز</label>
                        <input id="tenant-upload-btn" type="file" accept="image/*" onChange={(e)=>handleFileUpload(e,'tenant')} style={{display: 'none'}} />
                      </div>
                    </div>
                  </div>
                  {/* قسم الرخصة والـ AI - تم حل مشكلة السفاري هنا */}
                  <div style={styles.photoSection}>
                    ...
                    <div style={styles.photoBox}>
                      ...
                      <div style={styles.photoActions}>
                        ...
                        <label htmlFor="license-upload-btn" style={styles.btnSecondary}>📂 رفع للـ AI</label>
                        <input id="license-upload-btn" type="file" accept="image/*" onChange={(e)=>handleFileUpload(e,'license')} style={{display: 'none'}} />
                      </div>
                    </div>
                  </div>
                </div>
